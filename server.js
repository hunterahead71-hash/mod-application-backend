...
// Bot login with better error handling
async function loginBot() {
  ...
  console.log(`🔑 Token exists (${token.length} chars)`);
  
  // Validate token format
  if (!token.startsWith("MT") && !token.startsWith("NT")) {
    console.error("❌ Invalid token format! Should start with 'MT' or 'NT'");
    return false;
  }
...
if (process.env.DISCORD_BOT_TOKEN) {
  loginBot().then(success => {
    if (success) {
      ...
      console.log("⚠️ Bot failed to connect. Admin features will not work.");
    }
  });
} else {
...
async function sendDMToUser(discordId, title, description, color, footer = null) {
  try {
    ...
    if (error.code === 50007) {
      console.log(`📵 User ${discordId} has DMs disabled`);
    }
...
async function assignModRole(discordId) {
  console.log(`🎯 Attempting to assign mod role to ${discordId}`);
  ...
  try {
    ...
    console.error('❌ Error assigning mod role:', error.message);
...
app.get("/debug/bot-test", async (req, res) => {
  try {
    ...
    const token = process.env.DISCORD_BOT_TOKEN;
    const isValidFormat = token.startsWith("MT") || token.startsWith("NT");
